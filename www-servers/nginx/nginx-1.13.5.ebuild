...
# Tarantool
HTTP_UPSTREAM_TARANTOOL_MODULE_PV="2.5-r2"
...
HTTP_UPSTREAM_TARANTOOL_MODULE_WD="${WORKDIR}/ngx_http_upstream_tarantool-${HTTP_UPSTREAM_TARANTOOL_MODULE_PV}"
...
SRC_URI="https://nginx.org/download/${P}.tar.gz
	...
	nginx_modules_http_upstream_tarantool? ( ${HTTP_UPSTREAM_TARANTOOL_MODULE_URI} -> ${HTTP_UPSTREAM_TARANTOOL_MODULE_P}.tar.gz )
...
src_unpack() {
	...
	if use nginx_modules_http_upstream_tarantool; then
		mv "${HTTP_UPSTREAM_TARANTOOL_MODULE_UNPACK}" "${HTTP_UPSTREAM_TARANTOOL_MODULE_WD}" || die
	fi
}
...
src_configure() {
	...
	if use nginx_modules_http_upstream_tarantool; then
		http_enabled=1
		myconf+=( --add-dynamic-module=${HTTP_UPSTREAM_TARANTOOL_MODULE_WD} )
	fi
